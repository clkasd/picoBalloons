...
#define APRS_COMMENT "https://picoballoon.ist PBF-3"
...
#define APRS_PARM3   ",Humidity"
#define APRS_UNIT3   ",Pct"
#define APRS_EQNS3   ",0,1,0"

#define APRS_PARM4   ",Pressure"
#define APRS_UNIT4   ",hPa"
#define APRS_EQNS4   ",0,1,0"
...
void APRSWrapper::tx_aprs()
{
    ...
    if(aprstxstatus == 0)
    {
        ...
        ptr += 2;
        ax25_base91enc(ptr, 2,frame.humidity);
        ptr += 2;
        ax25_base91enc(ptr, 2,frame.pressure);
        if(frame.alt > 1000)
        {
            ax25_frame(
    ...
    {
        char s[10];
        strncpy_P(s, PSTR(CALLSIGN), 7);
        if(frame.APRSSSID) snprintf_P(s + strlen(s), 4, PSTR("-%i"), frame.APRSSSID);
        ax25_frame(
                   frame.APRSCallSign, frame.APRSSSID,
                   APRS_DEVID, 0,
                   0, 0,
                   APRS_PARM1 APRS_PARM2 APRS_PARM3 APRS_PARM4,
                   s);
        aprstxstatus = 2;
    }
    else if(aprstxstatus == 2)
    {
        ...
        ax25_frame(
                   ...
                   APRS_DEVID, 0,
                   0, 0,
                   APRS_UNIT1 APRS_UNIT2 APRS_UNIT3 APRS_UNIT4,
                   s);
        aprstxstatus = 3;
    }
